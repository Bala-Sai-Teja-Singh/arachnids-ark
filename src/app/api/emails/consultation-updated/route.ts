...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: senderEmail,
      to: [recipientEmail],
      subject: `CONSULTATION UPDATED: ${slotDate} at ${slotTime} - ArachnidsArk`,
      html: emailHtml.replace('Consultation Scheduled!', 'Consultation Updated!')
                     .replace('Your expert consultation has been scheduled.', 'Your expert consultation has been rescheduled. Please find the updated details below:'),
    });
